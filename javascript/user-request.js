...
function uniRequestSend() {
  ...
  let phone = $(form + ' input[name="phone"]').val().replace(/[ \-()]/g,'');
	
	$.ajax({
		url: 'index.php?route=extension/module/uni_request/mail',
		type: 'post',
		data: $(form+' input, '+form+' textarea').serialize()+'&location='+encodeURIComponent(window.location.href)+'&gaid='+gaid+'&page_title='+document.title,
		dataType: 'json',
		beforeSend: function() {
			$('.modal-request__btn.btn.btn-primary.btn-lg').button('loading');
		},
		complete: function() {
			$('.modal-request__btn.btn.btn-primary.btn-lg').button('reset');
		},
        ...
        dataLayer.push({
          ...
          'leadsUserData': {
            'phone_number': phone // передаємо динамічно телефон клієнта
          }
        });
